...
public class semanticRule {
    ...
    // ÓïÒåÆ¬¶Î3 :E->E+E{E.addr=newtemp();gen(E.addr=E1.addr+E2.addr);}
...
//        	 return "error 1 " +slrTreeArray.get(child1).getPos()+" "+ charTable.get(num1).getChara() + "(" + charTable.get(num1).getType() + ") ";
    ...
    // ÓïÒåÆ¬¶Î11,B->{S.next=newlabel();}S B
    ...
    // ÓïÒåÆ¬¶Î13,P->{ P1.true=P.true; P1.false = newlabel();}P1 or P2
    ...
    // ÓïÒåÆ¬¶Î14,P->P1 or{label(P1.false); P2.true=P.true; P2.false = P.false;} P2
    ...
    // ÓïÒåÆ¬¶Î15,P->{ P1.false=P.false; P1.true = newlabel();}P1 and P2
    ...
    // ÓïÒåÆ¬¶Î16,P->P1 and {label(P1.true); P2.true=P.true; P2.false = P2.false;}P2
    ...
    // ÓïÒåÆ¬¶Î17,P->not { P1.true=P.false; P1.false = P.true ;}P1
    ...
    // ÓïÒåÆ¬¶Î18,P->({ P1.true=P.true; P1.false = P.false ;} P1 )
...
//        System.err.println("s2: " + s2);
    ...
    //
    // ÓïÒåÆ¬¶Î19,P->E1 > E2  {gen (if E1.addr > E2.addr goto B.true);gen(goto B.false);}
    ...
    // ÓïÒåÆ¬¶Î20 P->true {gen(goto P.true);}
    ...
    // ÓïÒåÆ¬¶Î21 P->false {gen(goto P.false);}
...
//    // 
//    // ÓïÒåÆ¬¶Î22,24:S->id=E;{p=lookup(id.lexeme);if p== nil then
...
//    }
    // 
    // ÓïÒåÆ¬¶Î22,24:S->G=E;{p=lookup(id.lexeme);if p== nil then
    // error;S.CODE=E.code||gen(p¡¯=¡¯E.addr);}
    public String semanticRule_22(ArrayList<RreeSemanticRecord> treeSemanticRecord, ArrayList<LRTree> slrTreeArray,
            ArrayList<CharTable> charTable, ArrayList<FourAddr> fourAddr, int treeNodeNum) {
        int childID = Integer.parseInt(slrTreeArray.get(treeNodeNum).getChildId()[0]);
    ...
    // ÓïÒåÆ¬¶Î24,S->if {P.true=newlabel();P.false=S.next;} P {
    ...
    // ÓïÒåÆ¬¶Î25,S->if P { {label(P.true);S1.next=S.next} S1 }
    ...
    // ÓïÒåÆ¬¶Î26 S->if {P.true=newlabel();P.false=newlabel();}P { S1 } else {S2 }
    ...
    // ÓïÒåÆ¬¶Î27 S->if P { {label(P.true);S1.next=S.next}S1 } else { S2 }
    ...
    // ÓïÒåÆ¬¶Î28 S->if P { S1 } else { {label(P.false);S2.next=S.next}S2 }
    ...
    // ÓïÒåÆ¬¶Î29,S->while{S.begin=newlabel();label(S.begin);P.true=newlabel();P.false=S.next;}
    // P {S}
    ...
    // ÓïÒåÆ¬¶Î30,S->while P {{label(P.true);S1.next=S.begin;} S1 }
    ...
    // ÓïÒåÆ¬¶Î35,L->G [ E ] {L.array=G.addr;if L.array==null then error;
    ...
    // ÓïÒåÆ¬¶Î37,36 : X->a {T.type=a;X.width=4;} //intÐÍ
    ...
    // ÓïÒåÆ¬¶Î39,S->while P { S1 {gen(goto S.begin);} }
    ...
    // ÓïÒåÆ¬¶Î40,S->if P { S1 {gen(goto S.next);}} else { S2 }
